...
/**
 * CleanDashboard.java.
 * Implements ReadingListener — no static state, no direct controller calls.
 */
public class CleanView implements DistanceModel.ReadingListener {
    ...
    private void updateUI(DistanceReading reading) {
        ...
        gaugePanel.setValue(reading.getDistance());
        applyStatusStyle(statusBadge, reading.getStatus());
        updateAlertBanner(reading.getStatus());
        updateChartColor(reading.getStatus());

        avgLabel.setText(Math.round(model.getAverage())     + " cm");
        minLabel.setText(Math.round(model.getMin())         + " cm");
        maxLabel.setText(Math.round(model.getMax())         + " cm");
        samplesLabel.setText(String.valueOf(model.getSampleCount()));

        tableModel.insertRow(0, new Object[]{
            reading.getFormattedTimestamp(),
            reading.getDistance() + " cm",
            reading.getStatus()
        });
    ...
    private void updateAlertBanner(String status) {
        switch (status) {
            case "CRITICAL":
                alertLabel.setText("Critical object detected — distance below " + (int)model.getMin() + " cm");
                alertBanner.setBackground(CRIT_BG);
                alertBanner.setBorder(BorderFactory.createMatteBorder(0,0,1,0,CRIT_DOT));
                alertLabel.setForeground(CRIT_TEXT);
                ...
                break;
            case "WARNING":
                alertLabel.setText("Object in warning zone");
    ...
    private JPanel buildBody() {
        ...
        gbc.gridx = 0; gbc.weightx = 0; gbc.ipadx = 250;
    ...
    private JPanel buildLeftColumn() {
        ...
        gt.setBorder(new EmptyBorder(0, 0, 8, 0));
        gaugeCard.add(gt);
    ...
    private JPanel buildRightColumn() {
        ...
        ch.setOpaque(false);
        ch.setBorder(new EmptyBorder(0, 0, 10, 0));
        JLabel ct = new JLabel("Distance Timeline");
        ct.setFont(new Font("Segoe UI", Font.BOLD, 13));
        ct.setForeground(TEXT_MAIN);
        JLabel live = new JLabel("● Live");
        live.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        live.setForeground(SAFE_DOT);
        ch.add(ct, BorderLayout.WEST);
        ch.add(live, BorderLayout.EAST);
        chartCard.add(ch, BorderLayout.NORTH);
    ...
    static class RadialGaugePanel extends JPanel {
        ...
        RadialGaugePanel() {
            ...
            setOpaque(false);
            anim = new Timer(16, e -> { animated += (target - animated) * 0.10; repaint(); });
            anim.start();
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.drawString(val, cx - fm.stringWidth(val)/2, cy+7);
    ...
    static class RadarIconWidget extends JComponent {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.drawOval(1,1,17,17); g2.drawOval(5,5,9,9); g2.fillOval(8,8,4,4);
            g2.dispose();
        }
    }
}
